...
exports.getBasicInfo = async (id, options) => {
  ...
  try {
    ...
    if (parsedBody.reload === 'now' && !reqOptions.headers['x-youtube-identity-token']) {
      ...
      body = await fetch(jsonEndpointURL, reqOptions).then(body => body.text());
 ...
 */
const getDashManifest = (url, options) => new Promise(async (resolve, reject) => {
  try {
    ...
    const xml = new parser().parseFromString(body);
